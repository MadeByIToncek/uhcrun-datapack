...
execute positioned ~ ~1 ~ if block ~ ~ ~ warped_stem unless entity @e[type=area_effect_cloud,tag=TC_Log,distance=..0.1] if entity @e[type=area_effect_cloud,tag=TC_Log,distance=..5] positioned ~ ~-1 ~ run function treecapitator:z_run/trees/warped/validate/main